...
Like ci.yml's own placeholder-integrity job, checks (b) and (c) only make
sense on the pristine upstream template - a personalized fork is SUPPOSED
to have replaced the sentinels with real data, so those two classes skip
unless GITHUB_REPOSITORY matches the upstream repo (mirroring ci.yml's
`if: github.repository == 'MadsLorentzen/ai-job-search'` gate). Check (a),
which only inspects ci.yml's grep patterns, runs everywhere.
"""
import os
import unittest
...
UPSTREAM_REPO = "MadsLorentzen/ai-job-search"
IS_UPSTREAM = os.environ.get("GITHUB_REPOSITORY") == UPSTREAM_REPO
SKIP_REASON = (
    "pristine-sentinel checks only apply to the upstream template repo; "
    "a personalized fork is expected to have replaced them with real data"
)
...
class TestCvSentinelsAreDataLocated(unittest.TestCase):
    ...
    @unittest.skipUnless(IS_UPSTREAM, SKIP_REASON)
